 ...
 * most beginners learn on. The active playbook step lights up the exact control(s) it
 * touches (a bass knob, the crossfader…), so the coaching points at the user's hands.
 *
 * activeStep updates only at bar boundaries (a handful of times per transition), so a
 * plain React re-render is correct here — no imperative 60fps path needed.
 */
...
function RoundBtn({ cx, cy, label, active, ring }: { cx: number; cy: number; label: string; active: boolean; ring?: string }) {
  const r = 17;
  return (
    <g>
...
function Deck({ side, ox, on, dirOf }: SideProps & { ox: number }) {
  ...
  return (
    <g>
      ...
      <VFader x={tempoX} yTop={262} h={96} label="TEMPO" active={on(`${side}.tempo`)} dir={dirOf(`${side}.tempo`)} />

      {/* transport, outer corner */}
      <RoundBtn cx={transportX} cy={272} label="CUE" active={on(`${side}.cue`)} />
      <RoundBtn cx={transportX} cy={330} label="▶" active={on(`${side}.play`)} ring="#3A5A4E" />
    </g>
  );
}

/** One mixer channel strip: trim, 3-band EQ, CFX filter (bottom), cue, channel fader. */
function ChannelStrip({ side, cx, on, dirOf }: SideProps & { cx: number }) {
  return (
    <g>
      <PlainKnob cx={cx} cy={84} label="TRIM" />
      <Knob cx={cx} cy={120} label="HI" active={on(`${side}.hiEQ`)} dir={dirOf(`${side}.hiEQ`)} />
      <Knob cx={cx} cy={158} label="MID" active={on(`${side}.midEQ`)} dir={dirOf(`${side}.midEQ`)} />
      <Knob cx={cx} cy={196} label="LOW" active={on(`${side}.lowEQ`)} dir={dirOf(`${side}.lowEQ`)} />
      <Knob cx={cx} cy={234} label="CFX" active={on(`${side}.filter`)} dir={dirOf(`${side}.filter`)} />
      <Btn x={cx - 16} y={258} w={32} h={20} label="♪" active={on(`${side}.cue`)} />
      <VFader x={cx} yTop={290} h={78} label={side} active={on(`${side}.channelFader`)} dir={dirOf(`${side}.channelFader`)} />
    </g>
  );
}

export function FLX4Map({ playbook, activeStep }: FLX4MapProps) {
  ...
  const summary = controls
    .map((c) => `${PART_LABEL[c.part]}${c.dir ? (c.dir === "up" ? " ▲" : " ▼") : ""}${c.target !== "center" ? ` · ${c.target}` : ""}`)
    .join("   +   ");
  ...
  return (
    ...
    >
      ...
      <svg viewBox="0 0 900 480" width="100%" style={{ display: "block" }} role="img" aria-label={summary ? `FLX4: ${summary}` : "DDJ-FLX4 controller"}>
        ...
        <Deck side="A" ox={14} on={on} dirOf={dirOf} />
        ...
        <ChannelStrip side="A" cx={414} on={on} dirOf={dirOf} />
        <ChannelStrip side="B" cx={486} on={on} dirOf={dirOf} />
        ...
        <Deck side="B" ox={556} on={on} dirOf={dirOf} />
      </svg>
    </div>
  );
}
